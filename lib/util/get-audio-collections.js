...
module.exports = (rssAudioFiles) => {
    return new Promise((resolve, reject) => {
        s3.listObjects(bucketParams, (err, data) => {
            ...
            const responseFiles = data.Contents.filter((object) => object.Key !== bucketParams.Prefix);
            const currentFiles = responseFiles.map(responseFile => responseFile.Key.split(bucketParams.Prefix)[1]);
